...
private _vehicles = _this select {alive _x && {private _ammo = [_x] call EFUNC(common,getVehicleAmmo); _ammo != -1 && {_ammo < 1} && {_x isKindOf "AllVehicles"} && {!(_x isKindOf 'CAManBase')}};
