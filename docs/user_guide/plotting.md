...
You can cycle between different distance and azimuth formats using the <kbd>Ctrl</kbd>+<kbd>R</kbd> and <kbd>Ctrl</kbd>+<kbd>T</kbd> keys, which can be customized in the CBA Keybinding settings. 
...
Next to the distance, the travel time is shown in the format M:SS or H:MM:SS, based on the currently selected speed. You can change the speed by pressing <kbd>Shift</kbd>+<kbd>R</kbd> by default.
